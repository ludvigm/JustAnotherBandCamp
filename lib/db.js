    ...
    return new Promise(function (resolve, reject) {
        checkIfUserExists(input.username)
            .then(function (result) {
                if (result) {
                    connection.query('SELECT username, password FROM ' + USER + ' WHERE username=\'' + input.username + '\''
                        , function (err, result) {
                            if (err) reject(err);
                            if (result[0]) {
                                if (input.password == result[0].password) {
                                    resolve(true);
                                } else {
                                    resolve(false);
                                }
...
function registerUser(user, instrument, interest, contact) {
    return new Promise(function (resolve, reject) {
        checkIfUserExists(user.username)
            .then(function (res) {
                if (!res) {                         //IF user does NOT exist

                    var usr = {username: user.username, password: user.password, age: user.age, band: user.band};
                    // user INSERT
                    connection.query('INSERT INTO ' + USER + ' SET ?', usr, function (err) {
                        if (err) throw err;
                    });

                    // instrument INSERT
                    var inst = {type: instrument.type, skill: instrument.skill, username: user.username};
                    connection.query('INSERT INTO ' + INSTRUMENTS + ' SET ?', inst, function (err) {
                        if (err) throw err;
                    });

                    // interest INSERT
                    var intr = {genre: interest.genre, username: user.username};
                    connection.query('INSERT INTO ' + INTERESTS + ' SET ?', intr, function (err) {
                        if (err) throw err;
                    });

                    // contact INSERT
                    var cont = {phone: contact.phone, email: contact.email, username: user.username};
                    connection.query('INSERT INTO ' + CONTACTS + ' SET ?', cont, function (err) {
                        if (err) throw err;
                    });
                    resolve(true);
                } else {
...
function updateUser(username, user) {
    ...
    connection.query('UPDATE ' + USER + ', ' + INSTRUMENTS + ', ' + INTERESTS + ', ' + CONTACTS + ' SET ' + USER + '.username=\'' + user.username + '\', ' +
        ...
        USER + '.genre=\'' + user.type + '\', ' + CONTACTS + '.phone=\'' + user.phone + '\', ' +
