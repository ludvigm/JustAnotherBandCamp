...
function initDB() {
  ...
  connection.query('CREATE TABLE IF NOT EXISTS ' + INSTRUMENTS + '(type varchar(255), skill int, username varchar(255), FOREIGN KEY (username) REFERENCES ' + USER + '(username));', function(err) {
    if(err) throw err;
  });
  console.log(INSTRUMENTS + ' table created.');

  connection.query('CREATE TABLE IF NOT EXISTS ' + INTERESTS + '(genre varchar(255), username varchar(255), FOREIGN KEY (username) REFERENCES ' + USER + '(username));', function(err) {
    if(err) throw err;
  });
  console.log(INTERESTS + ' table created.');

  connection.query('CREATE TABLE IF NOT EXISTS ' + CONTACTS + '(phone int, email varchar(255), username varchar(255), FOREIGN KEY (username) REFERENCES ' + USER + '(username));', function(err) {
    if(err) throw err;
  });
   ...
   ' ON ' + USER + '.username=' + CONTACTS + '.username', function(err, result) {
    ...
    for (var i in result){
      ...
      var profile = {
          ...
          age : res.age,
          band: res.band,
          instrumenttype: res.type,
          instrumentskill: res.skill,
          interestgenre: res.genre,
...
/*
  connection.query('SELECT * FROM ' + USER, function(err, result) {
    ...
    for (var i in result){
      var res = result[i];
      console.log('First user result: ' + result);

      profile.username = res.username;
      profile.password = res.password;
      profile.age      = res.age;
      profile.band     = res.band;
      listOfProfiles.push(profile);
    }

    // INSTRUMENTS

    for(var i in listOfProfiles){
      connection.query('SELECT * FROM ' + INSTRUMENTS + ' WHERE username='+listOfProfiles[i].username+'', function(err, result) {
        if(err) throw err;
        for (var k in result){
          var res = result[k];
          listOfProfiles[i].instrumenttype  = res.type;
          listOfProfiles[i].instrumentskill = res.skill;
        }
      });
    }
